...
int main(int argc, const char** argv)
{
    ...
    auto pos_id = render.load_positions(pos);
    auto ind_id = render.load_indices(ind);

    int key = 0;
    int frame_count = 0;
    if (command_line) {
        render.clear(rst::Buffers::Color | rst::Buffers::Depth);
        render.set_model(get_model_matrix(angle));
        render.set_view(get_view_matrix(eye_pos));
        render.set_projection(get_projection_matrix(45, 1, 0.1, 50));

        render.draw(pos_id, ind_id, rst::Primitive::Triangle);
        cv::Mat image(700, 700, CV_32FC3, render.frame_buffer().data());
